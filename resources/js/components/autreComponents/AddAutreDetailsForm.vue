                  ...
                  <input
                    ...
                    :value="
                      Math.ceil((o.prix_total.$model =
                        o.puht.$model * o.qte.$model * (1 + tva / 100)))
                    "
                  />
...
export default {
  ...
  methods: {
    ...
    async enregistrer(){
        this.$v.$touch();
            if (!this.$v.stocks.$each.$error && this.stocks.length > 0) {
                await axios.post(`/geststock/autres/add-to-stock/${this.cid}`,{stocks:this.stocks}).then((response)=>{
