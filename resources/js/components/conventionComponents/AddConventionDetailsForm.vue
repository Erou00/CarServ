<template>
  <div class="container-fluid">
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(o, index) in $v.conventionDetails.$each.$iter" :key="index">
...
export default {
  ...
  methods: {
    ...
    async enregistrer(){
        this.$v.$touch();
            if (!this.$v.conventionDetails.$each.$error && this.conventionDetails.length > 0) {
                await axios.post(`/geststock/conventions/add-to-stock/${this.mid}`,
                {conventionDetails:this.conventionDetails}).then((response)=>{
