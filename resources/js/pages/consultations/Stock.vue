        ...
        <table class="table" id="stock-table" ref="exportable_table">
          ...
          <tbody>
            <tr v-for="(s, index) in pageOfItems" :key="index">
              <td>{{ (s.categorie) ? s.categorie.nom : ''}}</td>
              <td>{{ (s.souscategorie) ? s.souscategorie.nom : ''}}</td>
              <td>{{ (s.marque) ? s.marque.nom : ''}}</td>
  ...
  computed: {
    ...
    stocksList(){
        return this.stocks.filter(s => {
                  return s.id == this.product_id || s.categorie_id === this.categorie_id
  ...
  methods: {
    ...
    async ExportPdf(){
        ...
        para.innerHTML = ` <div>
                            ...
                            <img src="http://127.0.0.1:8000/public/assets/images/rym.png"
                               style='margin-bottom:5px; height:150px'/>
                            <img src="http://127.0.0.1:8000/public/assets/images/tgr.png"
                            style='margin-bottom:5px; height:150px'/>
                            </div>
                                <div style='display:flex;justify-content: space-between;'>
                                            <div></div>
