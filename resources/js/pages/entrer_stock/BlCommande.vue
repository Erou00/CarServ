<template>
  <div>
      ...
      <div class="form-group row">
        ...
        <div class="col-2">
          <div class="input-group">
            <input
              ...
              id="date"
              :value="commande ? commande[0].date_commande : ''"
            />
          </div>
        </div>
        ...
        <div class="col-3">
          <input
            ...
            placeholder=""
            id="no_marche"
            :value="commande ? commande[0].fournisseur.nom : ''"
          />
        </div>
      </div>
    </fieldset>
    ...
    <fieldset class="form-group p-3 mt-2">
      <legend class="p-2">Bon de Commande</legend>
      <div class="form-group row">
        <label for="text1" class="col-2 col-form-label text-end">N° BL:</label>
        <div class="col-4">
          <input
            type="text"
            class="form-control mb intputHeight"
        ...
        <label for="text1" class="col-2 col-form-label text-end">Date:</label>
        <div class="col-4">
          <input
            type="date"
            class="form-control mb intputHeight"
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(o, index) in $v.blDetails.$each.$iter" :key="index">
              ...
              <td><input type="text" class="form-control" readonly :name="'blDetails['+index+'][qte]'"  :value="o.qte.$model "></td>
...
export default {
  ...
  computed: {
    commandes() {
      return Object.values(this.$store.getters.commandesList);
    },
    magasins() {
  ...
  methods: {
    async setStocks(id) {
      if (id != null) {
        await this.$store.dispatch("displayAllCommandeDetails", id);
        this.opened = [];
        this.commande = this.$store.getters.commandeById;
        console.log(this.commande);
        this.bl.fournisseur_id = this.commande[0].fournisseur_id;
        this.bl.commande_id = this.commande[0].id;
        this.bl.magasin_id = this.commande[0].magasin_id;
        this.bl.sous_magasin_id = this.commande[0].sous_magasin_id;
    ...
    calcule() {
      let date_1 = new Date(this.commande.date_commande);
      ...
      this.bl.retard = days(date_1, date_2);
      if (this.bl.retard >= 0) {
        this.retardMsg = "Il reste en cours " + this.bl.retard + " jour";
      } else {
        this.retardMsg =
          "Le delai a été depassé par " + Math.abs(this.bl.retard) + " jour";
      }
    },
    toggle(s) {
  ...
  mounted() {
    this.$store.dispatch("displayAllCommandes");
